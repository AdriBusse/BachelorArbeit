...
describe('FetchMany', () => {
    it('load Sub and then access Post from Cache', async () => {
        ...
        render(<RouterContext.Provider value={createMockRouter({ pathname: '/r/[sub]' })}><MockedProvider mocks={[mock]} cache={cache}>
            <SubPage subName="Streetfood" />
        </MockedProvider></RouterContext.Provider>)
        await waitForResponse()

        //rendert Komponente Sub
        const headingPost = screen.getByText("Hottes Tacos in Mexico");
        expect(headingPost).toBeInTheDocument();
        //rendert Komponente Post
        const headingSub = screen.getByText("Streetfood");
        ...
        //Problem: Rootquery beinhaltet nicht den Query um Post abzufragen
        //rendert Post ohne Mock
        // const headingPostPage = screen.getByText("Hottes Tacos in Mexico");
        // expect(headingPostPage).toBeInTheDocument();
    ...
    it('die 2. Abfrage kommt aus dem Cache', async () => {
        ...
        //Daten werden aus dem Query geladen
        render(<RouterContext.Provider value={createMockRouter({})}><MockedProvider cache={cache} mocks={[mock]}>
            <SubPage subName="Streetfood" />
        </MockedProvider></RouterContext.Provider>)
        ...
        //rendert Sub Komponente
        const headingPost1 = screen.getByRole('link', { name: "Hottes Tacos in Mexico" });
        expect(headingPost1).toBeInTheDocument();
        //rendert Komponente Post
        const headingSub1 = screen.getByRole('heading', { name: "Streetfood" });
        expect(headingSub1).toBeInTheDocument();
